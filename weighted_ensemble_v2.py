...
def weighted_ensemble(x, e, w, cb, b, prop_out, propagator, split_merge, config_binner, ensemble_classifier, binner, calc_observables, nrounds, walkers_per_bin):
    ...
    for r in range(nrounds):
        ...
        x_md, prop_out_md = propagator.propagate(x, w)
        ...
        observables.append(calc_observables(x_last, x_md, e_last, e_md, w, cb_last, cb_md, b_last, b_md, propagator, prop_out_last, prop_out_md))

        #Split and merge trajectories
        (w, x, e, b, cb, prop_out) = split_merge(w, b_md, (x_md, e_md, b_md, cb_md, prop_out_md), walkers_per_bin)
    ...
    return x, e, w, cb, b, prop_out, propagator, observables  
...
def calc_observables_1(x_last, x, e_last, e, w, cb_last, cb, b_last, b, propagator, prop_out_last, prop_out):
    ...
    if prop_out is not None and prop_out_last is not None:
        #TODO add other correction factor from lab notebook
        mtd_transition_weights = np.sqrt(np.divide(prop_out, prop_out_last))
    else:
    ...
    return (trj_config_weighted, trj_weighted, cb_transitions, bin_transitions, mtd_weights, prop_out, mtd_transition_weights, len(w))
...
def we_histogram_msm(state, params):
    ...
    #unpack inputs
    x, e, w, cb, b, prop_out, propagator, cumulative_observables, cumulative_aggregate_time, cumulative_molecular_time = state
    ...
    #run dynamics
    x, e, w, cb, b, prop_out, propagator, new_observables = weighted_ensemble(x, e, w, cb, b, prop_out, propagator, split_merge, config_binner, ensemble_classifier, binner, calc_observables, nrounds, walkers_per_bin)
    ...
    return (x, e, w, cb, b, prop_out, propagator, observables, cumulative_aggregate_time, cumulative_molecular_time), (cumulative_aggregate_time, cumulative_molecular_time, est_bin_pops, eqp_msm), cumulative_aggregate_time >= aggregate_simulation_limit
...
def sampler_we(system_args, resource_args, bin_args, sampler_params):
    ...
    initial_state = (x0, e0, w0, cb0, b0, prop_out_0, propagator0, cumulative_observables0, 0, 0) #the final 0 is the initial aggregate simulation time
